...
bin=${root}/data/de-en/bin
repo=${root}/concat_models
ckpt_path=${root}/ckpt4
dropout=0.3
attention_dropout=0.1
activation_dropout=0.1
total_num_update=10000
wandb_project=reproduce-mega-new
model_size=base
    ...
    do
        ...
        do
            ...
            then 
                ...
                echo "${a}-${n}-${m}-${dropout}-${model_size}[${src}-${tgt}] running"
                ckpt=${ckpt_path}/${a}-${n}-${m}-${dropout}-${model_size}[${src}-${tgt}]
                mkdir -p $ckpt
                sbatch sh/${src}-${tgt}/train.sh \
                    --a=$a \
                    --t=reg \
